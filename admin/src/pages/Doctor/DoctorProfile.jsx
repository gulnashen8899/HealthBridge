...
const DoctorProfile = () => {
  ...
  const {dToken,profileData,setProfileData,getProfileData,backendURL}=React.useContext(DoctorContext)
  ...
  return profileData &&(
    <div>
      <div className='flex flex-col gap-4 m-5'>
        <div>
          <img className='bg-[#07BC]/80 w-full sm:max-w-64 rounded-lg' src={profileData.image} alt="" />
        </div>
    ...
    {
          isEdit ? <button onClick={updateProfile} className='px-4 py-1 border border-[#07BC] text-sm rounded-full mt-5 hover:bg-[#07BC] hover:text-white transition-all'>Save</button>
          :<button onClick={()=>setIsEdit(true)} className='px-4 py-1 border border-[#07BC] text-sm rounded-full mt-5 hover:bg-[#07BC] hover:text-white transition-all'>Edit</button>
